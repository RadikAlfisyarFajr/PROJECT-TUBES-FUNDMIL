    ...
    <main class="min-w-0 flex-1 overflow-x-hidden">
        <header class="flex h-[64px] items-center justify-between bg-white px-6 lg:px-[34px]">
            ...
            <div class="flex w-full items-center justify-between gap-[24px]">
                <form action="{{ route('mustahik.index') }}" method="GET" class="flex h-[36px] w-[318px] max-w-full items-center gap-3 rounded-full bg-[#eef2f0] px-4">
        ...
        <section class="px-6 pb-10 pt-[35px] lg:px-[34px]">
            <div class="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
                ...
                <a href="{{ route('mustahik.create') }}" class="inline-flex h-[64px] w-full items-center justify-center gap-[18px] rounded-[15px] bg-[#0b751f] text-[15px] font-black leading-[20px] text-white shadow-[0_12px_20px_rgba(8,117,31,0.28)] sm:w-[236px]">
                    <span class="flex h-[22px] w-[22px] items-center justify-center rounded-full bg-white text-[18px] text-[#0b751f]">+</span>
                    Tambah Mustahik
                </a>
            </div>
            ...
            <div class="mt-[35px] grid gap-[18px] sm:grid-cols-2 xl:grid-cols-4">
                <section class="rounded-[13px] bg-white px-[22px] py-[22px] shadow-sm ring-1 ring-[#e7eeea]">
            ...
            <form action="{{ route('mustahik.index') }}" method="GET" class="mt-[31px] grid max-w-full gap-3 rounded-[14px] bg-[#eef2f0] p-[5px] lg:grid-cols-[minmax(0,1fr)_minmax(150px,210px)_minmax(140px,190px)_48px]">
                ...
                <button class="grid h-[42px] place-items-center rounded-[11px] bg-[#0b751f] text-white shadow-sm" type="submit" aria-label="Filter">
            ...
            <section class="mt-[24px] overflow-hidden rounded-[13px] bg-white shadow-sm ring-1 ring-[#e3ebe6]">
                <div class="flex flex-col gap-2 border-b border-[#edf1ef] px-[24px] py-[20px] md:flex-row md:items-center md:justify-between">
                    ...
                    <span class="rounded-full bg-[#98f091] px-4 py-2 text-xs font-black text-[#0b751f]">{{ number_format($mustahik->total(), 0, ',', '.') }} data</span>
                </div>
                <div class="w-full">
                    <table class="w-full table-fixed text-left">
                        ...
                        <thead>
                            <tr class="bg-[#fafbf9] text-[10px] font-black uppercase tracking-[.16em] text-[#303b34]">
                                ...
                                <tr class="hover:bg-[#fbfdfb]">
                                    <td class="px-4 py-[18px] lg:px-5">
                                        <div class="flex items-center gap-4">
                                            <span class="grid h-[40px] w-[40px] shrink-0 place-items-center rounded-full bg-green-100 text-sm font-black text-[#0b751f]">{{ strtoupper($initials ?: 'M') }}</span>
                                            <div class="min-w-0">
                                                <p class="truncate text-[15px] font-black">{{ $row->nama }}</p>
                                                <p class="mt-1 truncate text-xs font-semibold text-[#7b8780]">{{ $row->kontak ?: 'Kontak belum diisi' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-3 py-5 lg:px-4">
                                        <span class="inline-flex max-w-full items-center rounded-full px-3 py-1 text-[10px] font-black uppercase {{ $badgeKategori[$row->kategori_asnaf] ?? 'bg-gray-100 text-gray-700' }}">
                                    ...
                                    <td class="px-4 py-5">
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route('mustahik.show', $row) }}" class="grid h-9 w-9 place-items-center rounded-[10px] bg-blue-50 text-blue-700 transition hover:bg-blue-100" aria-label="Detail" title="Detail">
                                            ...
                                            <a href="{{ route('mustahik.edit', $row) }}" class="grid h-9 w-9 place-items-center rounded-[10px] bg-amber-50 text-amber-700 transition hover:bg-amber-100" aria-label="Edit" title="Edit">
                                            ...
                                            <form action="{{ route('mustahik.destroy', $row) }}" method="POST" onsubmit="return confirm('Hapus data mustahik ini?')">
                                                @csrf
                                                @method('DELETE')
                                                <button class="grid h-9 w-9 place-items-center rounded-[10px] bg-red-50 text-red-700 transition hover:bg-red-100" aria-label="Hapus" title="Hapus" type="submit">
