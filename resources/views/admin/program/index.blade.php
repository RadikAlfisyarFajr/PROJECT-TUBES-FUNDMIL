    ...
    <aside class="fixed inset-y-0 left-0 z-20 hidden w-[254px] bg-white lg:block">
        ...
        <nav class="mt-[62px] space-y-[9px] px-4 text-[14px] font-semibold text-[#41546a]">
            @foreach ($menuItems as [$label, $icon, $url])
                <a href="{{ $url }}"
                   class="flex h-[48px] items-center gap-[17px] rounded-[16px] px-[18px] {{ $label === 'Program Penyaluran' ? 'bg-[#f4f8f6] font-black text-[#0b751f] shadow-[inset_4px_0_0_#0b751f]' : '' }}">
    ...
    <main class="min-h-screen flex-1 lg:ml-[254px]">
        <header class="flex h-[64px] items-center justify-between bg-white px-6 lg:px-[34px]">
            ...
            <div class="hidden items-center gap-[26px] md:flex">
                ...
                <div class="text-right">
                    <p class="text-[13px] font-black leading-none">Amil Zakat</p>
                    <p class="mt-[7px] text-[9px] font-bold uppercase tracking-wide text-[#89938e]">Administrator</p>
                </div>
                <div class="h-[40px] w-[40px] rounded-full bg-[linear-gradient(135deg,#0f7d33,#f0c58a)] ring-2 ring-white"></div>
            </div>
        </header>
        ...
        <section class="px-6 pb-10 pt-[35px] lg:px-[34px]">
            <div class="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
                <div>
                    <h2 class="text-[30px] font-black leading-tight tracking-[-0.02em]">Manajemen Program Penyaluran</h2>
            ...
            <div class="mt-[35px] grid gap-[24px] lg:grid-cols-2">
                <div class="h-[205px] rounded-[13px] bg-white px-[32px] py-[31px] shadow-sm ring-1 ring-[#e7eeea]">
                    ...
                    <p class="mt-[40px] text-[36px] font-black leading-none tracking-[-0.03em]">Rp {{ number_format($totalDana, 0, ',', '.') }}</p>
                ...
                <div class="h-[205px] rounded-[13px] bg-white px-[32px] py-[31px] shadow-sm ring-1 ring-[#e7eeea]">
                    ...
                    <p class="mt-[40px] text-[36px] font-black leading-none tracking-[-0.03em]">Rp {{ number_format($alokasiAktif, 0, ',', '.') }}</p>
                ...
                @foreach ($rows as $index => $program)
                    ...
                    <article class="min-h-[266px] rounded-[13px] bg-white px-[24px] py-[24px] shadow-sm ring-1 ring-[#e3ebe6]">
                        <div class="flex items-start justify-between">
                            ...
                            <span class="rounded-full px-[12px] py-[5px] text-[9px] font-black uppercase {{ $isSelesai ? 'bg-[#ddddda] text-[#5e645f]' : 'bg-[#98f091] text-[#0b751f]' }}">{{ $program->status }}</span>
                        </div>
                        ...
                        <div class="mt-[17px] flex items-center justify-between text-[12px]">
                            ...
                            <div class="flex items-center gap-3 font-black">
                                <a class="{{ $isSelesai ? 'text-[#9b9f9b]' : 'text-[#0b751f]' }}" href="#">{{ $isSelesai ? 'Arsip »' : 'Detail »' }}</a>
                                @if (! $isSample)
